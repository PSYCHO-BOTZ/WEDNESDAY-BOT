...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        m=await message.reply_sticker("CAACAgUAAxkBAAEPTk9j-b0cXv0Lng4PizsAAdlFD2_uyiMAAi8JAAIJAAGJVkdELc7PTi69LgQ")
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "ʙᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ", url=invite_link.invite_link
                )
            ]
        ]
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_chat_action(enums.ChatAction.TYPING)
        m=await message.reply_sticker("CAACAgUAAx0CSoZ7NwABEEqUY_ml4vgJcIuRXeLBW9HZF9QR8rsAAi8JAAIJAAGJVkdELc7PTi69LgQ")
        await asyncio.sleep(1)
        await m.delete()
        await message.reply_photo(
            photo=random.choice(PICS),
